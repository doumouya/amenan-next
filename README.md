...
3. **`--spacing: 0.25rem`** makes Tailwind's base step *the* minor grid, so every integer utility is on-rhythm by construction and arbitrary values are the only escape — which the spacing census catches.
4. **The five regions are ALWAYS present.** An unfilled region shows an empty state, never a collapse — a console's identity is its constant chrome.
5. **`retracted` is a STATE, not a component.** Splitting it meant two places to get the same chrome wrong.
6. **Surfaces are kept alive.** Hide with `visibility:hidden` + `inert` + `aria-hidden`; never `display:none`, which drops layout and resets inner scroll offsets.
7. **No persisted state may shape the first client render.** One text mismatch anywhere makes React regenerate the tree and strip the pre-paint theme attributes.
...
minted here, and **every `@custom-variant` a component uses is declared in the theme tier** — an
undeclared variant compiles to nothing at all, silently, which no typecheck or unit test can see.
